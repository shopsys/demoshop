...
use App\Model\PickUpPlace\Exception\PickUpPlaceNotFoundException;
...
class PickUpPlaceRepository
{
    ...
    public function findAllActiveByTransportType($transportType)
    {
        ...
        $pickUpPlaceQueryBuilder
            ->andWhere('pup.transportType = :transportType')
            ->setParameter('transportType', $transportType)
            ->andWhere('pup.active = TRUE')
            ->orderBy('NORMALIZE(pup.name), NORMALIZE(pup.city), NORMALIZE(pup.street), pup.postCode');
        return $pickUpPlaceQueryBuilder->getQuery()->execute();
    }
